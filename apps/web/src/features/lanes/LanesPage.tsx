...
import { lanes as staticLanes } from './lanes.data'
...
export function LanesPage() {
  ...
  const { data: apiLanes } = useQuery<ApiLane[]>({
    queryKey: ['lanes'],
    queryFn:  lanesApi.list,
    retry:    false,
  })

  const usingApi = Array.isArray(apiLanes) && apiLanes.length > 0

  const totalLoads = usingApi
    ? apiLanes.reduce((a, l) => a + l.metrics.reduce((s, m) => s + m.shipmentCount, 0), 0)
    : staticLanes.reduce((a, l) => a + l.loads, 0)

  const laneCount = usingApi ? apiLanes.length : staticLanes.length
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Active Lanes',       value: laneCount.toString() },
          { label: 'Total Loads (12mo)', value: totalLoads.toString() },
          { label: 'Avg $/Mile',         value: '$5.91' },
          { label: 'Trending Up',        value: usingApi ? '—' : `${staticLanes.filter(l => l.trend === 'up').length} lanes` },
        ].map((s) => (
